...
class CartPage extends Component {
		...
		return cart.map((val) => {
			return (
				<tr>
					...
					<td>{val.name}</td>
					<td>
						<img src={val.image} alt="img tidak ditemukan" height="50px" />
					</td>
	...
	checkOutAlert = () => {
		Swal.fire({
			...
			confirmButtonText: "Ya, saya mau membayar belanjaan",
		}).then((result) => {
			if (result.isConfirmed) {
				Swal.fire("Terhapus!", "File anda telah dihapus", "Sukses");
				this.checkOut();
			}
		});
	};
